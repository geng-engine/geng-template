use crate::{prelude::*, render::post::PostRender};

use geng_utils::conversions::Vec2RealConversions;
use geng_utils::task::Task;
...
impl<T: 'static> LoadingScreen<T> {
    pub fn new(
        ...
        let options = preferences::load(crate::OPTIONS_STORAGE).unwrap_or_default();
        Self {
            geng: geng.clone(),
            ...
            min_load_time: if insta_load { 0.0 } else { 2.5 },
            real_time: 0.0,
            texts: vec![
                "Loading assets...",
            ...
            current_text: 0,
            text_timer: Bounded::new_max(1.2),
        }
    }
...
impl<T: 'static> geng::State for LoadingScreen<T> {
    ...
    fn draw(&mut self, framebuffer: &mut ugli::Framebuffer) {
        ugli::clear(
            framebuffer,
            Some(crate::render::BACKGROUND_COLOR),
            None,
            None,
        );
        ...
        let buffer = &mut self
            .post_render
            .begin(framebuffer.size(), crate::render::BACKGROUND_COLOR);
        ...
        let t = geng_utils::interpolation::smoothstep(t);
        let fill_bar = fill_bar.extend_right((t - 1.0) * fill_bar.width());
        self.geng
            .draw2d()
            .quad(buffer, camera, load_bar, Color::WHITE);
        self.geng
            .draw2d()
            .quad(buffer, camera, fill_bar, Color::CYAN);
